...
def sweep_algorithm(x_coordinates, 
                    y_coordinates, 
                    vehicle_capacity, 
                    demand_dict, 
                    cost_matrix, 
    ...
    # single_route = []
    # for arc in all_arcs:
    #     single_route.append(arc)
    #     if all_arcs.index(arc) != 0 and arc[0] == 0:
    #         single_route.pop()
    #         routes_processed.append(single_route)
    #         single_route = [arc]
    #     if all_arcs.index(arc) == len(all_arcs) -1 :
    #         routes_processed.append(single_route)
    # print("routes_processed", routes_processed)
    # for rt in routes_processed:
    #     returned_routes = two_opt(route=rt,cost_matrix=cost_matrix,support_matrix=support_matrix)
    #     print(returned_routes)
    if len(routes) > fleet_size:
        return 1000000000000
    else:
        objective_value_sum = 0
        for ele in all_arcs:
            objective_value_sum += cost_matrix[ele[0],ele[1]] + support_matrix[ele[0],ele[1]]
        return objective_value_sum

def get_route_cost(route,cost_matrix,support_matrix):

    total_cost = 0
    for indnum in range(len(route)-1):
        total_cost += cost_matrix[route[indnum][0],route[indnum][1]] + support_matrix[route[indnum][0],route[indnum][1]] 
    return total_cost

def two_opt(route,cost_matrix,support_matrix):
    ...
    while improved:
        ...
        for i in range(1, len(route)-2):
            for j in range(i+1, len(route)):
                ...
                if get_route_cost(new_route,cost_matrix,support_matrix) < get_route_cost(best,cost_matrix,support_matrix):  
                        best = new_route
                        improved = True
        route = best
    return best
            ...
            for a in chosen_set:
                if a in V:
                    try:
                        ...
                        print("-------------------------something wrong in the route-------------------------")
                        return cutpool
            demand_sum = 0
            if len(chosen_set) >= 2:
...
def main(num_of_cust):
    ...
    upper_bound = sweep_algorithm(x_coordinates=xc,y_coordinates=yc,vehicle_capacity=vehicle_capacity,demand_dict=q,cost_matrix=cost_matrix, support_matrix=support_matrix,fleet_size=number_of_vehicles)
    print(upper_bound)
    best_sol , best_sol_value = BnBDFS(starting_node=strnode,MIPGap=0.1, mdl_upper_bound=upper_bound)
    algo_end = time.time()
    ...
    for i in N:
        ax.annotate('$q_%d=%d$' % (i, q[i]), (xc[i]+1, yc[i]),fontsize = 8)
    ax.text(31.195,121.40,"objective_value = " + str(truncate(best_sol_value, 3)))
...
def BnBDFS(starting_node,MIPGap, mdl_upper_bound):
    allcuts = list()
    allglmcuts = list()
    allcapcuts = list()
    mdl_lower_bound = 0
            ...
            if branch_instance.check_for_infeasible_or_onbounded():
                ...
                if branch_instance.check_for_integrality() and solution_value < mdl_upper_bound:
                    ...
                    if rota_olurlu:
                        ...
                        degismeme = 0
                        found_int_count += 1
                        figname = 'Support_normalized-n0%d.png' % n
